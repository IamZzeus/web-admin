...
export default function Dashboard() {
  ...
  const filteredTasks = tasks.filter((task) => {
    ...
    const matchesColab =
      filterColab === "" || String(task.user_id) === String(filterColab);

    // 👈 NUEVO: Filtra si tiene repetición activa
    const matchesRecurrence = showRecurringOnly
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      // Solo va al servidor si el navegador detecta que hay internet real
      if (navigator.onLine) {
        fetchDashboardData();
      }
    }, 15000);
  ...
  const openEditModal = async (task) => {
    ...
    if (user?.role === "colaborador" && !task.viewed_at) {
      try {
        ...
        if (!dataToSend.client_id || dataToSend.client_id === "")
          dataToSend.client_id = null;
        if (!dataToSend.recurrence_end || dataToSend.recurrence_end === "")
          dataToSend.recurrence_end = null;
        if (!dataToSend.time || dataToSend.time === "") dataToSend.time = null;
        if (
          !dataToSend.reschedule_reason ||
          dataToSend.reschedule_reason === ""
        )
          dataToSend.reschedule_reason = null;
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // 👇 NUEVO: Defensa anti-desconexión
    if (!navigator.onLine) {
      showToast(
        "❌ Sin conexión a internet. Espera a tener señal para guardar.",
        "error",
      );
      return;
    }
    ...
    dataToSend.append(
      "recurrence",
      typeof formData.recurrence === "object"
        ? JSON.stringify(formData.recurrence)
    ...
    try {
      if (modalMode === "create") {
        ...
        await api.post(`/tasks/${formData.id}`, dataToSend, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      ...
      // 👇 NUEVO: Manejo de Sesiones Zombie (Error 401 o 419 de Laravel)
      if (error.response?.status === 401 || error.response?.status === 419) {
        showToast(
          "🔒 Tu sesión caducó por inactividad. Recargando para proteger tus datos...",
          "error",
        );
        setTimeout(() => window.location.reload(), 2500); // Recarga la página sola
  ...
  const handleDeleteTask = async (id) => {
    if (
      !window.confirm(
        "⚠️ ¿Estás seguro de eliminar esta tarea definitivamente?",
      )
    )
      return;
                ...
                >
                  ...
                  <span
                    style={{
                      ...
                      borderRadius: "50%",
                      backgroundColor:
                        STATUS_CONFIG[task.status]?.color || "#ccc",
                      marginRight: "6px",
                  ...
                  <span
                    style={{
                      ...
                      color: "#fff",
                      fontWeight:
                        task.status === "en_proceso" ? "bold" : "normal",
                      verticalAlign: "middle",
                    }}
                  >
                    {/* 👇 MAGIA: Si la tarea tiene recurrencia, le pone el ícono 🔄 👇 */}
                    {task.recurrence && task.recurrence !== "none" && (
                      <span title="Se repite automáticamente">🔄 </span>
                    )}
            ...
            return (
              <div
                ...
                style={{
                  ...
                  opacity: task.status === "completada" ? 0.6 : 1,
                  border:
                    task.status === "en_proceso" ? "2px dashed black" : "none",
                }}
                onClick={() => openEditModal(task)}
                title={`Asignado a: ${colabInfo.name} | Estado: ${task.status}`}
              >
        ...
        {user?.role !== "colaborador" && (
          <>
            ...
            <button
              ...
              title={
                showRecurringOnly
                  ? "Ocultar tareas automatizadas"
                  : "Ver solo tareas automatizadas"
              }
              style={{
                padding: "10px",
                borderRadius: "6px",
          ...
          <div>
            ...
            <div
              style={{
                ...
                minHeight:
                  "60vh" /* 👈 LA MAGIA: Ocupa el 60% de la pantalla */,
                display: "flex",
                flexDirection: "column",
          ...
          <div style={styles.modalContent}>
            ...
            <div style={styles.tabContainer}>
              <button
                ...
                style={{
                  ...styles.tabButton,
                  ...(activeTab === "detalles" ? styles.tabButtonActive : {}),
                }}
              ...
              {modalMode === "create" && user?.role !== "colaborador" && (
                <button
                  ...
                  style={{
                    ...styles.tabButton,
                    ...(activeTab === "repetir" ? styles.tabButtonActive : {}),
                  }}
            ...
            >
              ...
              <div
                style={{
                  display: activeTab === "detalles" ? "flex" : "none",
                  flexDirection: "column",
                  gap: "15px",
                }}
              >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      style={{
                        ...styles.input,
                        cursor:
                          user?.role === "colaborador" ? "default" : "pointer",
                      }}
                      required
                      disabled={user?.role === "colaborador"} // 🔒 Bloqueado para técnicos
                    />
                ...
                >
                  ...
                  <div style={{ display: "flex", flexDirection: "column" }}>
                    ...
                    <select
                      ...
                      disabled={
                        user?.role === "colaborador" && !!formData.evidence_path
                      }
                    >
                    ...
                    {formData.status === "reagendar" && (
                      ...
                      >
                        ...
                        <textarea
                          ...
                          disabled={
                            false
                          } /* 👈 2. FIX: Quitamos el candado para que el técnico SIEMPRE pueda escribir */
                        />
                        {user?.role === "colaborador" && (
                          <small
                            style={{
                          ...
                          >
                            ⚠️ Escribe el motivo y guarda los cambios para
                            confirmar.
                          </small>
                        )}
                      </div>
                  ...
                  {user?.role === "colaborador" ? (
                    ...
                    <div>
                      <label style={{ ...styles.label, color: "#007bff" }}>
                        ...
                        {Object.keys(STATUS_CONFIG).map((statusKey) => {
                          ...
                          return (
                            <button
                              ...
                              type="button"
                              onClick={async () => {
                                // 👇 NUEVO: Defensa anti-desconexión para botones rápidos
                                if (!navigator.onLine) {
                                  showToast(
                                    "❌ Sin señal. Conéctate a internet para actualizar la tarea.",
                                    "error",
                                  );
                                  return;
                                }
                                ...
                                const necesitaEvidencia =
                                  ...
                                  statusKey === "finalizado_con_pendientes";
                                const tieneEvidenciaNueva =
                                  evidenceFile || evidenceNotes;
                                const teniaEvidenciaVieja =
                                ...
                                if (
                                  ...
                                  showToast(
                                    "⚠️ Agrega una foto o nota antes de finalizar.",
                                    "warning",
                                  );
                                  return;
                                }
                                ...
                                try {
                                  ...
                                  // 👇 NUEVO: Manejo de sesión para técnicos
                                  if (
                                    err.response?.status === 401 ||
                                    err.response?.status === 419
                                  ) {
                                    showToast(
                                      "🔒 Tu sesión caducó. Volviendo a conectar...",
                                      "error",
                                    );
                                    setTimeout(
                                    ...
                                    showToast(
                                      "❌ Error al registrar la operación. Revisa tu señal.",
                                      "error",
                                    );
                                  }
                                }
                              ...
                              style={{
                                ...
                                gap: "6px",
                                boxShadow: isActive
                                  ? `0 3px 8px ${config.color}60`
                                  : "none",
                                gridColumn:
                                  statusKey === "reagendar" ? "1 / -1" : "auto",
                              }}
                            >
                              {isActive && (
                ...
                <div>
                    ...
                    {formData.lat && formData.lng && (
                      <button
                        ...
                        style={{
                          padding: "10px 15px",
                          backgroundColor:
                            "#149004" /* Azul característico de Google */,
                          color: "white",
                          border: "none",
                          borderRadius: "6px",
                          cursor: "pointer",
                  ...
                  {showMap && isLoaded && (
                    <div style={{ marginTop: "10px" }}>
                      ...
                      >
                        👆 Haz clic en el mapa para colocar el pin exacto del
                        servicio:
                      </p>
                      <GoogleMap
                ...
                {user?.role === "colaborador" && formData.client && (
                  ...
                  >
                    ...
                    >
                      <span>
                        <strong>Empresa:</strong> {formData.client.name}
                      </span>
                      <span>
                        <strong>Contacto:</strong>{" "}
                        {formData.client.contact_person || "No especificado"}
                      </span>
                      <span>
                        <strong>📞 Teléfono:</strong>{" "}
                        <a
                          href={`tel:${formData.client.phone}`}
                ...
                {modalMode === "edit" && (
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      if (
                        formData.evidence_path &&
                        String(formData.evidence_path) !== "null" &&
                        String(formData.evidence_path).trim() !== ""
                      ) {
                      ...
                      if (
                        formData.evidence_notes &&
                        String(formData.evidence_notes) !== "null" &&
                        String(formData.evidence_notes).trim() !== ""
                      ) {
                        try {
                          ...
                          notesArray = Array.isArray(parsed)
                            ? parsed
                            : [parsed];
                        } catch {
                            ...
                            >
                              ...
                              {totalEvidences > 1 && (
                                ...
                                >
                                  ...
                                  <button
                                    ...
                                    onClick={() =>
                                      setEvidenceIndex((prev) =>
                                        Math.min(totalEvidences - 1, prev + 1),
                                      )
                                    }
                                    disabled={
                                      evidenceIndex === totalEvidences - 1
                                    }
                                    style={{
                                      border: "none",
                                      background: "#6c757d",
                              ...
                              {pathsArray[evidenceIndex] ? (
                                <div
                                  onClick={() =>
                                    setFullScreenImage(
                                      `https://api.gigafiber.mx/storage/${pathsArray[evidenceIndex]}?v=${Date.now()}`,
                                    )
                                  }
                                >
                                  <img
                              ...
                              {user?.role === "colaborador" ? (
                                <>
                                  ...
                                  >
                                    ...
                                    >
                                      ⚠️ Para subir esta evidencia, da clic en
                                      un "Estado de Actualización Rápida" o en
                                      el botón verde de "Guardar Cambios" de
                                      abajo.
                                    </span>
                                  </div>
                                </>
                                ...
                                >
                                  ...
                                  <span style={{ fontSize: "13px" }}>
                                    El técnico aún no ha subido evidencia.
                                  </span>
                                </div>
                              )}
                            </div>
                ...
                >
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      const isFinished = estadosFinales.includes(
                        formData.status,
                      );

                      // Si ya terminó, usamos su fecha de finalización. Si no, usa null para que el reloj siga corriendo en vivo.
                      const endTimePoint = isFinished
                        ? formData.completed_at || formData.updated_at
                        : null;
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {(() => {
                                ...
                                if (
                                  String(formData.creator_id) ===
                                  String(user?.id)
                                )
                                  return `Tú (${user?.role === "marca" || user?.role === "admin" ? "Admin" : user?.role.charAt(0).toUpperCase() + user?.role.slice(1)})`;

                                const creador = collaborators.find(
                                  (c) =>
                                    String(c.id) ===
                                    String(formData.creator_id),
                                );
                                if (creador) {
                                  const rolFormat =
                                    creador.role.charAt(0).toUpperCase() +
                                    creador.role.slice(1);
                          ...
                          {(() => {
                            ...
                            const modificador = collaborators.find(
                              (c) =>
                                String(c.id) === String(formData.updater_id),
                            );

                            // MAGIA: Solo lo dibuja SI el usuario existe y su rol es EXCLUSIVAMENTE 'supervisor'
                            if (
                              modificador &&
                              modificador.role === "supervisor"
                            ) {
                              return (
                                <span
                                  style={{
                          ...
                          >
                            <strong style={{ color: "#495057" }}>
                              👁️ Visto por el técnico:
                            </strong>
                            <span
                              style={{
                                color: formData.viewed_at
                                  ? "#28a745"
                                  : "#6c757d",
                                marginTop: "4px",
                              }}
                          ...
                          >
                            <strong style={{ color: "#495057" }}>
                              ▶️ Iniciada (En proceso):
                            </strong>
                            <span
                              style={{
                                color: formData.started_at
                                  ? "#17a2b8"
                                  : "#6c757d",
                                marginTop: "4px",
                              }}
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                <span style={{ color: "#555" }}>
                                  <strong>Tiempo de Ejecución</strong> (Inicio ➔
                                  Cierre):
                                </span>
                                <span
                                  style={{
                              ...
                              <div
                                style={{
                                  ...
                                  backgroundColor: isFinished
                                    ...
                                    : "#e3f2fd",
                                  borderRadius: "6px",
                                  border: `1px solid ${isFinished ? "#c3e6cb" : "#b6d4fe"}`,
                                }}
                              >
                                <span
                                  style={{
                                    color: isFinished ? "#155724" : "#0056b3",
                                    fontSize: "14px",
                                  }}
                                >
                                  <strong>
                                    {isFinished
                                      ? "Tiempo Final Congelado"
              ...
              {activeTab === "repetir" && (
                ...
                >
                  ...
                  >
                    ...
                    >
                      Configura la automatización. El sistema generará las
                      copias a la medianoche.
                    </p>
                    ...
                    >
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            if (val === "weekly_custom")
                              setFormData({
                                ...formData,
                                recurrence: { type: "weekly_custom", days: [] },
                              });
                            else if (val === "monthly_custom")
                              setFormData({
                                ...formData,
                                recurrence: {
                                  type: "monthly_custom",
                        ...
                        >
                          <option value="none">
                            No repetir (Una sola vez)
                          </option>
                          <option value="daily">Todos los días</option>
                          <option value="weekly_custom">
                            Semanal (Elegir días)
                          </option>
                          ...
                          <option value="monthly_custom">
                            Mensual (Elegir días exactos ej. 15 y 30)
                          </option>
                      ...
                      {typeof formData.recurrence === "object" &&
                        formData.recurrence.type === "weekly_custom" && (
                          <div
                            style={{
                              gridColumn: "1 / -1",
                              backgroundColor: "#fff",
                            ...
                            >
                              {[
                                ...
                                >
                                  <input
                                    type="checkbox"
                                    checked={formData.recurrence.days.includes(
                                      dia.num,
                                    )}
                                    onChange={(e) => {
                                      ...
                                      if (e.target.checked)
                                        actuales.add(dia.num);
                                      else actuales.delete(dia.num);
                                      setFormData({
                                        ...formData,
                                        recurrence: {
                                          ...formData.recurrence,
                                          days: Array.from(actuales),
                                        },
                                      });
                                    }}
                                  />{" "}
                      ...
                      {typeof formData.recurrence === "object" &&
                        formData.recurrence.type === "monthly_custom" && (
                          <div style={{ gridColumn: "1 / -1" }}>
                            <label style={styles.label}>
                              ¿Qué días del mes? (Ej: 15, 30)
                            </label>
                            <input
                              type="text"
                              style={styles.input}
                              ...
                              onChange={(e) => {
                                const numeritos = e.target.value
                                  .split(",")
              ...
              <div style={{ display: "flex", gap: "10px", marginTop: "15px" }}>
                ...
                {(user?.role !== "colaborador" ||
                  ...
                  formData.status ===
                    "reagendar" /* 👈 EL FIX: Forzamos a que aparezca al reagendar */ ||
                  !formData.evidence_path ||
                  String(formData.evidence_path) === "null" ||
                  String(formData.evidence_path) === "undefined" ||
                  String(formData.evidence_path).trim() === "") && (
                  <button
                    type="submit"
                    disabled={isSaving}
        ...
        >
          <div
            style={{
              ...
              flexDirection: "column",
              maxHeight:
                "85vh" /* 👈 Límite estricto de altura para la ventana */,
            }}
            onClick={(e) => e.stopPropagation()}
          >
            {/* CABECERA FIJA */}
            ...
            >
              {(() => {
                ...
                const rutinas = tasks
                  .filter(
                    (t) =>
                      ...
                      (t.title
                        .toLowerCase()
                        .includes(recurringSearchTerm.toLowerCase()) ||
                        (t.description &&
                          t.description
                            .toLowerCase()
                            .includes(recurringSearchTerm.toLowerCase()))),
                ...
                return rutinas.map((task) => {
                  ...
                  else if (task.recurrence === "bimonthly")
                    freqLabel = "Bimestral";
                  else if (
                    typeof task.recurrence === "string" &&
                    task.recurrence.includes("weekly_custom")
                  )
                    freqLabel = "Semanal (Días específicos)";
                  else if (
                    typeof task.recurrence === "string" &&
                    task.recurrence.includes("monthly_custom")
                  )
                    freqLabel = "Mensual (Fechas específicas)";
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        onClick={() =>
                          setExpandedRecurringTaskId(
                            isExpanded ? null : task.id,
                          )
                        }
                        style={{
                          padding: "12px",
                          backgroundColor: isExpanded ? "#f4f6f9" : "#fff",
                          display: "flex",
                          justifyContent: "space-between",
                          alignItems: "center",
                          cursor: "pointer",
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              {task.recurrence_end || "Indefinido"}
                            </span>
                          </div>
                          ...
                          >
                            <button
                              onClick={() => {
                                ...
                                openEditModal(task);
                                setTimeout(() => setActiveTab("repetir"), 50);
                              }}
                              style={{
                                flex: 1,
                                padding: "10px",
                            ...
                            <button
                              onClick={async () => {
                                if (
                                  !window.confirm(
                                    "¿Detener automatización? Esta tarea se quedará en tu calendario con todo su historial, pero ya no se generarán copias en el futuro.",
                                  )
                                )
                                  return;
                                try {
          ...
          style={{
            ...
            backgroundColor:
              toast.type === "error"
                ...
                : toast.type === "warning"
                  ? "#ffc107" // Amarillo para advertencias
